...
const AddNewMenu = ({ setRefetch, categories }: AddNewMenuProps) => {
    ...
    async function handleAddDish(e: React.FormEvent) {
        ...
        try {
            const res = await axios.post("/api/dish/create", formData, {
            ...
            if (!res?.data?.isError) {
                ...
                toast.error(res?.data?.message ?? "Error adding dish.")
            }
        } catch (err: any) {
